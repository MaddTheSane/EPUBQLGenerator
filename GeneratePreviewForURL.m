                               ...
                               CFDictionaryRef options)
{
  @autoreleasepool {
    ...
    if(plistRef) {
      maximumNumberOfLoadingHTML = [(__bridge NSNumber *)plistRef integerValue];
      CFRelease(plistRef);
    }
    key = (CFStringRef)@"MaximumNumberOfLoadingImage";
    plistRef = CFPreferencesCopyAppValue(key, appId);
    if(plistRef) {
      maximumNumberOfLoadingImage = [(__bridge NSNumber *)plistRef integerValue];
      CFRelease(plistRef);
    }
    ...
    NSXMLNode *charSetAttribute = [[NSXMLNode alloc] initWithKind:NSXMLAttributeKind];
